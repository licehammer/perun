...
public class urn_perun_user_attribute_def_def_login_namespace_eduteams_persistent_shadow extends urn_perun_user_attribute_def_def_login_namespace {
	...
	private final static String eduteamsAttributesPropertiesFile = "eduteams-attributes.properties";
	...
	public Attribute fillAttribute(PerunSessionImpl perunSession, User user, AttributeDefinition attribute) throws InternalErrorException, WrongAttributeAssignmentException {
		...
		if (attribute.getFriendlyName().equals(attrNameEduteams)) {
			String eduteamsScope = "@" + BeansUtils.getPropertyFromCustomConfiguration(eduteamsAttributesPropertiesFile, "scope");
			filledAttribute.setValue(sha1HashCount(user, eduteamsScope).toString() + eduteamsScope);
	...
	public void changedAttributeHook(PerunSessionImpl session, User user, Attribute attribute) throws InternalErrorException, WrongReferenceAttributeValueException {
		try {
			...
			if(userNamespace.equals("eduteams-persistent-shadow") && attribute.getValue() != null){
				String extSourceNameEduteams = BeansUtils.getPropertyFromCustomConfiguration(eduteamsAttributesPropertiesFile, "extSourceName");
